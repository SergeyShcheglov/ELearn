...
//  ELearn
//
//  Created by Sergey Shcheglov on 02.05.2022.
//
...
struct BlobView: View {
    ...
    func path(in rect: CGRect, x: Double, x2: Double) -> Path {
        ...
        path.move(to: CGPoint(x: 0.9923*width, y: 0.425*height))
        path.addCurve(to: CGPoint(x: 0.6344*width*x2, y: height),
...
struct BlobShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        let height = rect.size.height
        path.move(to: CGPoint(x: 0.9923*width, y: 0.425*height))
        path.addCurve(to: CGPoint(x: 0.6344*width, y: height), control1: CGPoint(x: 0.92554*width, y: 0.77749*height), control2: CGPoint(x: 0.91864*width, y: height))
